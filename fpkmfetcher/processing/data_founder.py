...
class GDCServer(object):
    ...
    def get_case_multiple_expands(self, params):
        ...
        for expand in expand_list[1:]:
            for case_number in range(len(data["data"]["hits"])):
                diagnose = self.get_case_by_id(
                    data["data"]["hits"][case_number]["case_id"], expand
                )
                data["data"]["hits"][case_number][expand] = diagnose["data"]["hits"][0][
                    expand
                ]
    ...
    def get_case_information(self, tumor_stage):
        try:
            ...
            _LOGGER.info(
                "Error occurred while getting cases information: {}".format(err)
            )
    ...
    def save_case_info(self, data, file_name):
        try:
            ...
            file_dir_name = (
                directory + "_information_" + file_name + file_time + ".json"
            )
    ...
    def get(self):
        ...
        for stage in self.__config["tumor_stages"]:
            _LOGGER.info("#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-# ")
            _LOGGER.info(
                "Searching files of {} in https://api.gdc.cancer.gov ...".format(stage)
            )
