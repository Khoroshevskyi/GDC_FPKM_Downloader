        ...
        for one_stage_folder in directory_contents:
            if len(one_stage_folder) > 5:
                if one_stage_folder[0:5] == "stage":
                    files_dict[one_stage_folder] = os.listdir(
                        main_dir + "/" + one_stage_folder
                    )  # self.list_full_paths(main_dir + "/" + one_stage_folder)
...
def main_arguments():
    ...
    if options["append"].lower() in ["true", "t"]:
        joiner.add_new_expression(
            options["caseid"],
            TUMOR_STAGE[options["stage"]],
            options["input"],
            options["output"],
        )
    else:
...
def main():
    ...
    joiner.join_fpkm_files(
        "E:/Przyrod-master/0_master_thesis/data/files/2",
        "E:/Przyrod-master/0_master_thesis/data/files/2/last_file.csv",
    )
